...
class OrutegoApp {
    ...
    bindEvents() {
        ...
        document.getElementById('closeExpandedBtn').addEventListener('click', this.closeExpandedView.bind(this));
    ...
    expandView() {
        document.querySelector('.main-layout').classList.add('expanded-layout');
        document.getElementById('closeExpandedBtn').classList.remove('hidden');
        ...
        if (this.googleMap) {
            ...
            // And after a delay to allow for transitions/rendering
            setTimeout(() => {
                google.maps.event.trigger(this.googleMap, 'resize');
                // Re-fit bounds if we have a route
                if (this.lastResult) {
                    const bounds = new google.maps.LatLngBounds();
                    bounds.extend(new google.maps.LatLng(this.lastResult.originCoords[0], this.lastResult.originCoords[1]));
                    bounds.extend(new google.maps.LatLng(this.lastResult.destinationCoords[0], this.lastResult.destinationCoords[1]));
    ...
    closeExpandedView() {
        document.querySelector('.main-layout').classList.remove('expanded-layout');
        document.getElementById('closeExpandedBtn').classList.add('hidden');
